...
function AccordionTrigger({ className, isLeft = false, children, ...props }) {
  return (
    <AccordionPrimitive.Header className="flex items-center gap-2">
      {isLeft && (
        <ChevronDownIcon className="text-muted-foreground data-[state=open]:text-secondary pointer-events-none size-4 shrink-0 translate-y-0.5 transition-transform duration-200" />
      )}
