...
/**
 * Normalizes the responsive prop to the keys expected by cva variants.
 * Accepts boolean or string ("true" | "false").
 * @param {boolean | "true" | "false" | undefined} responsive
 * @returns {"true" | "false"}
 */
function responsiveKey(responsive) {
  if (typeof responsive === "string")
    return responsive === "true" ? "true" : "false";
  return responsive ? "true" : "false";
}
...
/**
 * @typedef {Object} TypographyBaseProps
 * @property {string} [className]
 * @property {"h1"|"h2"|"h3"|"h4"|"h5"|"h6"|"p"|"blockquote"|"lead"|"large"|"small"|"muted"|"code"|"list"} [variant]
 * @property {"xs"|"sm"|"base"|"lg"|"xl"|"2xl"|"3xl"|"4xl"|"5xl"} [size]
 * @property {boolean|"true"|"false"} [responsive]
 * @property {boolean} [asChild]
 */

const Typography = React.forwardRef(
  (
    /** @type {TypographyBaseProps & React.ComponentPropsWithoutRef<any>} */
    { className, variant, size, responsive = true, asChild = false, ...props },
    ref
  ) => {
      ...
      <Comp
        ...
        className={cn(
          typographyVariants({
            variant,
            size,
            responsive: responsiveKey(responsive),
            className,
          })
        )}
        {...props}
...
const Heading = React.forwardRef(
  (
    /** @type {{level?: number, className?: string, responsive?: boolean|"true"|"false", asChild?: boolean}} */
    { level = 1, className, responsive, asChild = false, ...props },
    ref
  ) => {
